    ...
    async.waterfall([
        ...
        function(cb) {
            if (!req.oauth2.model.client.getRedirectUri(client))
                ...
                if (client === 'build-envato-com-test') {
                  req.oauth2.logger.debug(JSON.stringify({ error: 'Wrong RedirectUri provided', clientRedirectUri: req.oauth2.model.client.getRedirectUri(client), redirectUri: redirectUri }))
                }
