...
module.exports = function(oauth2, client, username, password, scope, pCb) {
    ...
    async.waterfall([
        ...
        function(cb) {
            oauth2.model.refreshToken.removeByUserIdClientId(oauth2.model.user.getId(user), oauth2.model.user.getId(client), function(err) {
                if (err)
        ...
        function(cb) {
            ...
            oauth2.model.refreshToken.save(refreshTokenValue, oauth2.model.user.getId(user), oauth2.model.user.getId(client), scope, function(err) {
                if (err)
